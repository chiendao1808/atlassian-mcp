# Codex Workflow Orchestrator Guide
...
3. Do not edit agent TOML files during an active workflow instance merely to change a model.
...
2. Load the referenced `state-model.yaml` as the authoritative state and transition definition.
...
5. Store the runtime state in a project-local location such as `.codex/orchestration/runtime/<workflow_id>.yaml` when persistence is required.
