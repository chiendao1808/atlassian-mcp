...
- `code_reviewer`: inspects the change set directly in its assigned thread, never spawns another agent, and produces structured findings.
...
- Review scope must be explicit: workspace diff, staged diff, commit range, branch comparison, or supplied files.
