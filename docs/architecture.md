...
The app loads shared process configuration first, then validates Jira and Bitbucket modules independently. A static configuration error in one module disables only that module and writes a sanitized warning to `stderr`; `stdout` is reserved for MCP protocol traffic.
